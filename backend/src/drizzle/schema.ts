...
export const enderecos = pgTable(
	"enderecos",
	{
		id: integer().primaryKey().generatedByDefaultAsIdentity({
			name: "enderecos_id_seq",
			startWith: 1,
			increment: 1,
			minValue: 1,
			maxValue: 2147483647,
...
export const tiposProdutos = pgTable(
	"tipos_produtos",
	{
		id: integer().primaryKey().generatedByDefaultAsIdentity({
			name: "tipos_produtos_id_seq",
			startWith: 1,
			increment: 1,
			minValue: 1,
			maxValue: 2147483647,
	...
	(table) => [
		...
		index("tipos_produtos_slug_idx").on(table.slug),
	],
);

export const imgsProdutos = pgTable(
	"imgs_produtos",
	{
		id: integer().primaryKey().generatedByDefaultAsIdentity({
			name: "imgs_produtos_id_seq",
			startWith: 1,
			increment: 1,
			minValue: 1,
			maxValue: 2147483647,
...
export const produtos = pgTable(
	"produtos",
	{
		...
		descricao: text(),
		corDestaque: varchar("cor_destaque"),
		oldImage: varchar("old_image", { length: 255 }),
		idTipo: integer("id_tipo").notNull(),
		imagemPrincipal: varchar("imagem_principal", { length: 255 }).notNull(),
		slug: varchar({ length: 255 }).notNull(),
	...
	(table) => [
		...
		index("produtos_slug_idx").on(table.slug)
	],
);
...
export const itensCarrinho = pgTable(
	...
	(table) => [
		uniqueIndex("itens_carrinho_id_carrinho_id_produto_idx").using(
			"btree",
			table.idCarrinho.asc().nullsLast().op("int4_ops"),
			table.idProduto.asc().nullsLast().op("int4_ops"),
		),
		foreignKey({
			columns: [table.idProduto],
			foreignColumns: [produtos.id],
...
export const usuarios = pgTable(
	"usuarios",
	{
		...
		nome: varchar({ length: 190 }).notNull(),
		senha: varchar({ length: 190 }).notNull(),
	},
	(table) => [unique("usuarios_email_key").on(table.email)],
);

export const carrinhos = pgTable(
	"carrinhos",
	{
		id: integer().primaryKey().generatedByDefaultAsIdentity({
			name: "carrinhos_id_seq",
			startWith: 1,
			increment: 1,
			minValue: 1,
			maxValue: 2147483647,
	...
	(table) => [
		foreignKey({
			columns: [table.idUsuario],
			foreignColumns: [usuarios.id],
			name: "carrinhos_id_usuario_fkey",
		}).onDelete("cascade"),
