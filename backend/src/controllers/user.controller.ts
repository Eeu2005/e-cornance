...
export async function refreshToken(jwt: string, refreshToken: string) {
  ...
  const newJwt = await createToken({ id: data.id, tipo: data.tipo, email: data.email, nome: data.nome });
  const newRefreshToken = await createRefreshToken(data.id);
  return {
    jwt: newJwt,
    refreshToken: newRefreshToken,
    user: { nome: data.nome, email: data.email, tipo: data.tipo }
  };
}
async function createToken({ id, tipo, email, nome }: jwtPayload) {
...
export async function createUser({ email, nome, senha }: userParams) {
  ...
  const jwt = await createToken({ id, tipo, email, nome });
  const refreshToken = await createRefreshToken(id);
  return { jwt, refreshToken, user: { nome, email, tipo } };
}
export async function login({
  ...
  return { jwt, refreshToken, user: { nome: user.nome, email: user.email, tipo: user.tipo } };
}
